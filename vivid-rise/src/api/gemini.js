...
const BASE_DB = {
  '강남·서초': {
    ...
    eat: {
      '한식': ['땀땀', '무월식탁', '청국장밥', '감성타코', '강남불백'],
      '일식': ['고에몬', '감탄성신', '오제제', '카츠8', '은행골'],
      '양식': ['미즈컨테이너', '마초쉐프', '을지다락 강남', '롤링파스타', '바비레드'],
      '중식': ['대가방', '홍콩반점0410', '초선과여포', '일일향', '차알'],
      '분식': ['보슬보슬', '남도분식', '장꼬방', '애플하우스', '신포우리만두'],
      '멕시칸': ['감성타코', '낙원타코', '도스타코스', '쿠차라', '훌리오'],
      '샐러드': ['피그인더가든', '알라보', '스윗밸런스', '샐러디', '크리스피프레시'],
      '디저트': ['노티드 청담', '랜디스도넛', '아우어베이커리', '썸띵어바웃커피', '트리오드']
    },
    do: {
  ...
  '용산·이태원': {
    ...
    eat: {
      '한식': ['몽탄', '남영돈', '바다식당', '이태원클라쓰 단밤'],
      '일식': ['오레노라멘', '멘야산다이메', '스시로'],
      '양식': ['부자피자', '다운타우너 한남', '파이프그라운드'],
      '중식': ['이태원 우육미엔', '자리', '마라토끼'],
      '분식': ['창화당', '남도분식', '한남동 떡볶이'],
      '멕시칸': ['바토스', '하시엔다', '코레아노스 키친'],
      '샐러드': ['샐러드셀러', '루트', '왓어샐러드'],
      '디저트': ['오월의종', '패션5', '써머레인']
    },
    do: {
  ...
  '종로·을지로': {
    ...
    eat: {
      '한식': ['광장시장 육회자매집', '진옥화할매원조닭한마리', '계림 닭도리탕'],
      '일식': ['종로돈부리', '갓덴스시', '오죽이네'],
      '양식': ['익선동 온천집', '경양식 1920', '을지다락'],
      '중식': ['안동장', '홍콩반점0410', '오구반점'],
      '분식': ['남도분식 익선', '종로분식', '맛나분식'],
      '멕시칸': ['엘쁠라또', '훌리오', '토마틸로'],
      '샐러드': ['어게인리프레쉬', '힐사이드테이블', '마치래빗'],
      '디저트': ['청수당', '익선동 쌀상회', '호랑이커피']
    },
    do: {
  ...
  '성수·건대': {
    ...
    eat: {
      '한식': ['소문난성수감자탕', '할머니의레시피', '성수다락'],
      '일식': ['탐광', '배키욘방', '호호식당'],
      '양식': ['팩피', '문화식당', '다로베'],
      '중식': ['전자방', '중앙감속기', '건대 양꼬치거리'],
      '분식': ['해피치즈스마일', '금금', '도산분식'],
      '멕시칸': ['갓잇', '와하카', '토마틸로'],
      '샐러드': ['잇샐러드', '르베지왕', '샐러디'],
      '디저트': ['어니언', '대림창고', '노티드 성수']
...
async function getDatabase() {
  ...
  try {
    ...
    rows.forEach(row => {
      // CSV 파싱: Region, Category, Name 순서라고 가정 (또는 시트 구조에 맞춰 수정 가능)
      // 현재 시트 구조를 유연하게 읽기 위해 아래와 같이 처리합니다.
      const cols = row.split(',').map(c => c.trim().replace(/^"|"$/g, ''));
      
      // 시트의 컬럼 순서가 [지역, 카테고리, 식당이름] 이라고 가정할 때:
      // (만약 시트가 다른 구조라면 이 부분을 수정하면 됩니다)
      const region = cols[0];   // 예: 강남·서초
      const category = cols[1]; // 예: 한식
      const name = cols[2];     // 예: 땀땀

      if (region && category && name && db[region]) {
        if (!db[region].eat[category]) {
          db[region].eat[category] = [];
        }
        // 기존 리스트에 없으면 추가 (중복 방지)
    ...
    console.log("✅ 구글 시트 데이터 병합 완료");
    CACHED_DB = db;
    return db;

  } catch (e) {
    console.warn("🚨 구글 시트 로드 실패 (기본 데이터 사용):", e);
    return BASE_DB;
  }
}
